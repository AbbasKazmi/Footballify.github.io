...
var run = async () => {
    const res = await fetch(`https://v3.football.api-sports.io/fixtures?date=2022-06-01`, {
        headers: {
       ...
       for (x=0; x<filtered[i].length;x++){
        ...
        if (lineupData.length===0) {
            ...
            for (let d=0; d<=lineupData[0].startXI.length-1; d++) {
                ...
                lineupParentHome.innerHTML=`${lineupData[0].startXI[d].player.number} | ` + lineupData[0].startXI[d].player.name  + ` (${lineupData[0].startXI[d].player.pos})`
                ...
                lineupParentAway.innerHTML=`(${lineupData[1].startXI[d].player.pos}) | ` + lineupData[1].startXI[d].player.name + ` ${lineupData[1].startXI[d].player.number} `
